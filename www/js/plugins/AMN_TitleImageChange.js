...
// ----------------------------------------------------------------------------
// Version
// 1.0.1 2023/09/02 「進行度反映」True時、セーブファイル無しで進行度のみ保存すると、タイトル画面でエラーが出る問題を修正。
// 1.0.0 2023/09/01 本家に、進行度変数の反映を「最新のセーブデータ」で行う設定を追加。
// ----------------------------------------------------------------------------
// [Twitter]: https://twitter.com/ikyex/
//=============================================================================
 ...
 * @author トリアコンタン（改変：天音 衣月）
 *
 * @param 進行度の反映
 * @desc 進行度の反映は最新のセーブデータを使用する。
 * (使用するのは進行度変数です)
 * @on はい
 * @off いいえ
 * @default false
...
(function () {
  "use strict";
  var pluginName = "AMN_TitleImageChange";
  ...
  paramTitleBgms = paramTitleBgms
    ...
    .reverse();
  const paramGradeBoolean = getParamString(["GradeBoolean", "進行度の反映"]);
  //=============================================================================
  // DataManager
  //  ゲーム進行状況を保存します。
  //=============================================================================
  ...
  Scene_Title.prototype.changeTitleImage = function () {
    ...
    var Savefile_Exists = DataManager.isAnySavefileExists();
    if ($dataSystem.originalTitle1Name !== undefined) {
      $dataSystem.title1Name = $dataSystem.originalTitle1Name;
    }
    //===========================================================================
    //≪改造部分(タイトル画像)≫「進行度反映」の状態で分岐。
    //  false 従来のプラグイン処理
    //  True  最新セーブファイルIDのgradeVariableで判定。
    //===========================================================================
    if (paramGradeBoolean === "true" && Savefile_Exists === true) {
      var Global_Info = DataManager.loadGlobalInfo();
      var Save_ID = DataManager.latestSavefileId();
      var Grade_Variable = Global_Info[Save_ID].gradeVariable;
      for (var i = 0, n = paramTitleGrades.length; i < n; i++) {
        if (paramTitleImages[i] && Grade_Variable == paramTitleGrades[i]) {
  ...
  Scene_Title.prototype.changeTitleBgm = function () {
    ...
    if (paramGradeBoolean === "true" && Savefile_Exists === true) {
      ...
      var Grade_Variable = Global_Info[Save_ID].gradeVariable;
      for (var i = 0, n = paramTitleGrades.length; i < n; i++) {
        if (paramTitleBgms[i] && Grade_Variable == paramTitleGrades[i]) {
