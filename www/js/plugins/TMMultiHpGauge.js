...
(function () {
  ...
  Window_MapHpGauge.prototype.isNeedRefresh = function () {
    ...
    if (actor) {
      ...
      for (var i = 0; i < gauges.length; i++) {
        ...
        switch (gauges[i].type) {
          ...
          case "VN":
            if (
              gaugeParam.param !==
                $gameVariables.value(gauges[i].param + this._windowId) ||
              gaugeParam.max !==
                $gameVariables.value(gauges[i].max + this._windowId)
            ) {
              this._gaugeParams[i].param = $gameVariables.value(
                gauges[i].param + this._windowId
              );
              this._gaugeParams[i].max = $gameVariables.value(
                gauges[i].max + this._windowId
              );
              result = true;
            }
            break;
      ...
      if (
        goldPosition.width > 0 &&
        (goldDisplay === this._windowId || goldDisplay === -1) &&
  ...
  Scene_Map.prototype.update = function () {
    ...
    if (
      $gameSystem.mapHpGaugeX() !== this._mapHpGaugeWindows[0].baseX() ||
